...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        if report.report_name == "hr_expense_sheet.report_expense_sheet":
            ...
            for expense_sheet in expense_sheets:
                ...
                attachments = self.env["ir.attachment"].search(
                    [
                        ("res_id", "in", expense_sheet.expense_line_ids.ids),
                ...
                for attachment in self._prepare_local_attachments(attachments):
                    if attachment.mimetype == "application/pdf":
                        ...
                        # In case the attachment is not a pdf we will create a new PDF
                        # from the template "report_expense_img"
                        # And then append to the stream. By doing so, the attachment
                        # is put on a new page with the name of the expense associated
                        # to the attachment
                        data["attachment"] = attachment
                        attachment_prep_stream = self._render_qweb_pdf_prepare_streams(
                            "hr_expense.report_expense_img",
                            data,
                            res_ids=res_ids,
                        )
                        attachment_stream = attachment_prep_stream[expense_sheet.id][
                            "stream"
                        ]
                    ...
                    try:
                        ...
                        expense_sheet._message_log(
                            body=self.env._(
                                "The attachment (%(attachment_name)s) has not been "
                                "added to the report due to the following "
                                "error: '%(error)s'",
