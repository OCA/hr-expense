...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _get_default_advance(self, fields_list):
        ...
        return BaseModel.default_get(self, fields_list)

    @api.model
    def _get_valid_return_advance_account_types(self):
        return ["asset_current"]

    @api.model
    def _default_get_return_advance(self, fields_list):
        ...
        if "line_ids" in fields_list and "line_ids" not in res:
            ...
            valid_account_types = self._get_valid_return_advance_account_types()
            for line in lines:
            ...
            if len(lines.company_id.root_id) > 1:
                raise UserError(
                    _(
                        "You can't create payments for entries belonging to "
                        "different companies."
                    )
                )
            if len(set(available_lines.mapped("account_type"))) > 1:
                raise UserError(
                    _(
                        "You can't register payments for both inbound and "
                        "outbound moves at the same time."
                    )
                )
    ...
    def default_get(self, fields_list):
        if self._context.get("hr_return_advance", False):
            return self._default_get_return_advance(fields_list)
    ...
    def expense_post_return_advance(self):
        ...
        expense_sheet = line_ids.expense_id.sheet_id
